...
class UploadedFile implements UploadedFileInterface
{
    ...
    {
        if (!is_dir($targetPath) || !is_readable($targetPath)) {
            throw new InvalidArgumentException(Constants::ERROR_BAD . "Path should readble and writable");
        }
        if ($this->moved) {
        ...
        if ($this->randomize) {
            $final = bin2hex(random_bytes(8)) . '.' . $this->getExtension();
        } else {
    ...
    /**
     * Returns the moved name of the uploaded file
     * @return string The moved file name
     */
